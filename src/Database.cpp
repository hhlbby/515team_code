...
bool Database::openDatabase(const std::string& filename) { return sqlite3_open(filename.c_str(), &db) == SQLITE_OK; }
bool Database::execute(const std::string& sql) {
    char* errMsg = nullptr;
    int rc = sqlite3_exec(db, sql.c_str(), nullptr, nullptr, &errMsg); // <- db, а не db_handle
    if (rc != SQLITE_OK) {
...
std::vector<User> Database::getAllUsers() {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        users.push_back(User(id,username,pwd,name,role));
    }
...
User* Database::authenticateUser(const std::string& username,const std::string& password){
    ...
    if(sqlite3_step(stmt)==SQLITE_ROW){
        ...
        std::string name=reinterpret_cast<const char*>(sqlite3_column_text(stmt,3));
        std::string role=reinterpret_cast<const char*>(sqlite3_column_text(stmt,4));
        user=new User(id,uname,pwd,name,role);
    }
    sqlite3_finalize(stmt);
    return user;
}
